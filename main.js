...
if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
        ...
        conn.logger.warn(`[ ⚠ ] Lost connection with server, reconnecting...`);
        process.send('reset');
        ...
        conn.logger.info(`[ ⚠ ] Reboot required, please reboot the server if there are any issues.`);
        //process.send('reset');
    } else if (reason === DisconnectReason.timedOut) {
        conn.logger.warn(`[ ⚠ ] Connection timed out, reconnecting...`);
        process.send('reset');
    } else {
        conn.logger.warn(`[ ⚠ ] Reason for disconnection unknown. ${reason || ''}: ${connection || ''}`);
        //process.exit();
    }
}
...
conn.spromote =  '*@user YOU ARE NOW ADMIN IN THIS GROUP!!*';
conn.sdemote =  '*@user NO LONGER ADMIN IN THIS GROUP!!*';
conn.sDesc =  '*THE NEW DESCRIPTION OF THE GROUP IS:*\n\n@desc';
conn.sSubject = '*THE NEW NAME OF THE GROUP IS:*\n\n@subject'; 
conn.sIcon =  '*THE PHOTO OF THIS GROUP WAS CHANGED!!*';
conn.sRevoke = '*NOW THIS IS THE NEW LINK OF THIS GROUP!!*\n\n*@revoke*';
...
if (existsSync(dir)) conn.logger.info(` IF UPDATED - '${filename}' SUCCESSFULLY`);
else {
...
if (err) conn.logger.error(`SYNTAX ERROR DETECTED | SYNTAX ERROR WHILE LOADING '${filename}'\n${format(err)}`);
else {
try {
const module = (await import(`${global.__filename(dir)}?update=${Date.now()}`));
global.plugins[filename] = module.default || module;
} catch (e) {
conn.logger.error(`THERE IS AN ERROR IT REQUIRES THE PLUGINS '${filename}\n${format(e)}'`);
} finally {
...
_quickTest()
.then(() => conn.logger.info(chalk.bold(`⌛ LOADING...\n`)))
.catch(console.error)