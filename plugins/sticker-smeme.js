...
let handler = async (m, {
    ...
    if (!/webp|image|video|gif|viewOnce/g.test(mime)) return m.reply(`Reply to media with the command\n\n${usedPrefix + command} <${topText ? topText : 'top text'}>|<${bottomText ? bottomText : 'bottom text'}>`)
    
    let img = await quotedMessage.download?.()
    let meme = "https://api.memegen.link/images/custom/" + encodeURIComponent(topText ? topText : '_') + "/" + encodeURIComponent(bottomText ? bottomText : '_') + ".png?background="
