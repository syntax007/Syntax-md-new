let handler = async (m, { conn, usedPrefix, args, command }) => {
  ...
  async function cekAFK(x){
    ...
    if (turn == turnNow && time == timeNow){
      ...
      conn.reply(m.chat,`*@${conn.war[m.chat][turn].user.split('@')[0]} while AFK (Fine -2500 HP)*\n\n.war player = player stats\n.attack @tag = attack opponent`,null,{contextInfo : {mentionedJid : [conn.war[m.chat][turn].user]}})
      await sleep(3000)
      // cek kalau mati
      if (conn.war[m.chat][turn].hp <= 0) {
        conn.reply(m.chat,`*@${conn.war[m.chat][turn].user.split('@')[0]} is dead because the HP (Health Point) runs out.*`,null,{contextInfo : {mentionedJid : [conn.war[m.chat][turn].user]}})
        // cek tim nya
        let playerTotal = 0
        let playerKalah = 0
        if (turn < 5){
          ...
          if (playerTotal == playerKalah){
            ...
            conn.reply(m.chat, `*TEAM A WINS BECAUSE TEAM B IS ALL IMPOSSIBLE*\n\n*TEAM A :*\n` + teamA.map((v, i )=> `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (+ Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) => `${conn.war[m.chat][i+5].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (- Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n`,m, {contextInfo: {
              mentionedJid: teamAB
            }})
            delete conn.war[m.chat]
            delete conn.war2[m.chat]
      ...
      await sleep(3000)
      conn.reply(m.chat,`*turn @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} to attack (Time 90 seconds)*\n\n.war player = player stats\n.attack @tag = attack opponent`,null,{contextInfo : {mentionedJid : [conn.war[m.chat][conn.war2[m.chat].turn].user]}})
      cekAFK(conn.war2[m.chat].turn)
    }
  }

  if (!(m.chat in conn.war)) return m.reply(`*There are no games in this group.*`)
  if (!conn.war2[m.chat].war) return m.reply(`*War has not started, when ".war start" to start the fight.*`)
  for (let i=0;i<10;i++){
    if (m.sender == conn.war[m.chat][i].user){
      if (i != conn.war2[m.chat].turn) {
        conn.reply(m.chat,`*Now is the turn @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} to attack.*`,m, {contextInfo : { mentionedJid : [conn.war[m.chat][conn.war2[m.chat].turn].user]}})
        cekAFK(conn.war2[m.chat].turn)
      }
    }
  }
  if (!args[0]) return m.reply(`*Tag the enemy to attack*\n*Type .war player*`)
  ...
  if (conn.war2[m.chat].turn < 5){
       ...
       let  target = args[0]
        ...
        for (let i=0;i<10;i++){
          if (conn.war[m.chat][i].user == target){
            ...
            conn.reply(m.chat,`*@${attacker.split('@')[0]} attack @${target.split('@')[0]} until his health is reduced ${pointAttacker * 500} (Remaining HP: ${conn.war[m.chat][i].hp})*\n\n*@${attacker.split('@')[0]} [${pointAttacker*10}%] - [${pointTarget*10}%] @.*`,m,{contextInfo : {mentionedJid : [attacker, target]}})
            await sleep(2000)
            if (conn.war[m.chat][i].hp <= 0) conn.reply(m.chat,`*@${target.split(`@`)[0]} already died in battle.*`,m, {contextInfo : {mentionedJid : [target]}})
            success = true
          }
        }
      }
    ...
    if (success == false) {
      return m.reply(`*Enter the correct list of game players boss.*\n\n*Check ".war player"*`)
    }else {
      for (let i=0;i<10;i++){
        if (m.sender == conn.war[m.chat][i].user){
  ...
  if (conn.war2[m.chat].turn < 5){
    ...
    if(userAktif == userMati){
      ...
      conn.reply(m.chat, `*TEAM A WINS BECAUSE TEAM B BLOCK ALL*\n\n*TEAM A :*\n` + teamA.map((v, i )=> `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (+ Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) => `${conn.war[m.chat][i+5].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (- Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n`,m, {contextInfo: {
        mentionedJid: teamAB
      }})
      delete conn.war[m.chat]
      delete conn.war2[m.chat]
    ...
    await sleep(2000)
    conn.reply(m.chat,`*@${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]}'s turn to attack (Time 90 seconds)*\n\n. war player = player stats\n.attack @tag = attack opponent`,m, {contextInfo : {mentionedJid: [conn.war[m.chat][conn.war2[m.chat].turn].user]}})
    cekAFK(conn.war2[m.chat].turn)
  }else {
    let userAktif = 0
    let userMati = 0
