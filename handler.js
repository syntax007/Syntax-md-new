...
if (m.text && user.banned && !isROwner) {
  ...
  if (user.bannedMessageCount < 3) {
    ...
    const messageText = `❰ ⚠️ ❱ *You are banned* ❰ ⚠️ ❱\n Notice (${messageNumber}/3)${user.bannedReason ? `\n*Motivo:* *${user.bannedReason}*` : ''}
*👉 You can contact the owner of the Bot if you think it is an error (HAVE PROOF) to deal with the reason for your ban*
...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `Hola *@${nk.from.split('@')[0]}*, las ${nk.isVideo ? 'videollamadas' : 'llamadas'} are not allowed, you will be blocked.\n\n If you accidentally called contact my creator to unblock you!`, false, {mentions: [nk.from]});
...
global.dfail = (type, m, conn) => {
    ...
    sourceUrl: `http://wa.me/917821096044`,
    mediaType: 1,
